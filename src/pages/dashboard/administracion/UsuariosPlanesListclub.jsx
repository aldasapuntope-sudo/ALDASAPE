...
import UsuarioPlanFormclub from "./componentes/UsuarioPlanFormclub";

export default function UsuariosPlanesListclub() {
  ...
  const fetchData = async () => {
    setCargando(true);
    try {
      const res = await axios.get(`${config.apiUrl}api/administracion/lplanes_usuarioclub`);
      setData(res.data);
    } catch (err) {
      console.error(err);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.put(
            `${config.apiUrl}api/administracion/eplanes_usuarioclub/${row.id}/estado`,
            { estado: newStatus }
          );
  ...
  return (
    <>
      ...
      <div className="container mt-4">
        ...
        {showForm && (
          <UsuarioPlanFormclub planUsuario={selected} onClose={handleCloseForm} />
        )}
      </div>
    </>
  );
