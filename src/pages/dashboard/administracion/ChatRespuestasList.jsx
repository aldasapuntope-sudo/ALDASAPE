...
import ChatRespuestaForm from "./componentes/ChatRespuestaForm";

export default function ChatRespuestasList() {
  ...
  const fetchRespuestas = async () => {
    setCargando(true);
    try {
      const res = await axios.get(`${config.apiUrl}api/administracion/lchat-respuestas`);
  ...
  const handleChangeStatus = async (id, newStatus) => {
    const confirm = await Swal.fire({
      title: newStatus ? "¿Activar respuesta?" : "¿Desactivar respuesta?",
      icon: "question",
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.put(
          `${config.apiUrl}api/administracion/echat-respuesta/${id}/estado`,
          { is_active: newStatus }
        );
        fetchRespuestas();
  ...
  const columns = [
    ...
    {
      name: "Palabras clave",
      selector: (row) => row.palabras_clave,
      sortable: true,
      wrap: true,
    },
    {
      name: "Respuesta",
      selector: (row) =>
        row.respuesta.length > 60
          ? row.respuesta.substring(0, 60) + "..."
          : row.respuesta,
      wrap: true,
    },
    {
