...
import CaracteristicaFormclub from "./componentes/CaracteristicaFormclub";

export default function CaracteristicaListclub() {
  ...
  const fetchCaracteristicas = async () => {
    setCargando(true);
    try {
      const res = await axios.get(`${config.apiUrl}api/administracion/lcaracteristicasclub`);
  ...
  const handleChangeStatus = async (id, newStatus) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.put(`${config.apiUrl}api/administracion/ecaracteristicaclub/${id}/estado`, {
          is_active: newStatus,
        });
        fetchCaracteristicas();
        Swal.fire(
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) =>
        row.icono ? (
          <img
            src={`${config.urlserver}iconosclub/${row.icono}`}
            alt="icono"
            width="16"
            height="16"
          />
  ...
  return (
    <>
      ...
      <div className="container mt-4">
        ...
        {showForm && (
          <CaracteristicaFormclub Caracteristica={selectedItem} onClose={handleCloseForm} />
        )}
      </div>
    </>
  );
