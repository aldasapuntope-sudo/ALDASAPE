...
export default function UsuarioPlanFormclub({ planUsuario, onClose }) {
    ...
    const fetchCombos = async () => {
      try {
        const [usuariosRes, planesRes] = await Promise.all([
          ...
          axios.get(`${config.apiUrl}api/administracion/lplanescomboxclub`),
        ]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (planUsuario) {
          await axios.put(
            `${config.apiUrl}api/administracion/aplanes_usuarioclub/${planUsuario.id}`,
            values
          );
          Swal.fire("Actualizado", "La asignación fue actualizada", "success");
        } else {
          await axios.post(
            `${config.apiUrl}api/administracion/rplanes_usuarioclub`,
            values
          );
