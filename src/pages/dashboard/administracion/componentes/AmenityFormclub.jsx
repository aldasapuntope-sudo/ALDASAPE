...
export default function AmenityFormclub({ Servicio, onClose }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        if (Servicio) {
          await axios.put(
            `${config.apiUrl}api/administracion/aamenitiesclub/${Servicio.id}`,
            values
          );
          Swal.fire("✅ Actualizado", "El servicio fue actualizado correctamente", "success");
        } else {
          await axios.post(`${config.apiUrl}api/administracion/ramenitiesclub`, values);
