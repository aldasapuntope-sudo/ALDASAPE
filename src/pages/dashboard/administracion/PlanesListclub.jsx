...
import PlanFormclub from "./componentes/PlanFormclub";

export default function PlanesListclub() {
  ...
  const fetchPlanes = async () => {
    setCargando(true);
    try {
      const res = await axios.get(`${config.apiUrl}api/administracion/lplanesclub`);
      setPlanes(res.data);
    } catch (error) {
  ...
  const handleChangeStatus = async (id, newStatus) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.put(`${config.apiUrl}api/administracion/eplanesclub/${id}/estado`, {
          is_active: newStatus,
        });
        fetchPlanes();
        Swal.fire(
  ...
  return (
    <>
      ...
      <div className="container mt-4">
        ...
        {showForm && <PlanFormclub plan={selectedPlan} onClose={handleCloseForm} />}
      </div>
    </>
  );
}
