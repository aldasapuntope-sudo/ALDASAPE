import React, { useEffect, useState } from 'react';
import { Row, Col, ListGroup, Card, Button } from 'react-bootstrap';
import axios from 'axios';
import { useUsuario } from '../../../context/UserContext';
import config from '../../../config';
import { FaWhatsapp, FaEnvelope } from 'react-icons/fa';
import { useLocation } from 'react-router-dom';
import BreadcrumbALDASA from '../../../cuerpos_dashboard/BreadcrumbAldasa';

export default function SoporteMensajesPage() {
  const { usuario } = useUsuario();
  const [mensajes, setMensajes] = useState([]);
  const [mensajeSeleccionado, setMensajeSeleccionado] = useState(null);
  const location = useLocation();

  useEffect(() => {
    if (!usuario) return;

    axios
      .get(`${config.apiUrl}api/misanuncios/mensajes-soporte/${usuario.usuarioaldasa.id}`)
      .then(res => {
        setMensajes(res.data);

        // ✅ Si viene un mensaje desde UserInfoBoxAldasa, seleccionarlo automáticamente
        if (location.state?.mensajeSeleccionado) {
          const msg = location.state.mensajeSeleccionado;
          setMensajeSeleccionado(msg);
        }
      })
      .catch(err => console.error('Error al cargar mensajes:', err));
  }, [usuario, location.state]);

  //console.log(mensajes);
  if (!usuario) return <p>Cargando usuario...</p>;

  

  return (
    <>
      <BreadcrumbALDASA />
      <Row style={{ height: '80vh', marginTop: '20px' }}>
        <Col md={4}>
          <Card style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Card.Header>Mensajes</Card.Header>
            <ListGroup
              variant="flush"
              style={{
                flex: 1,
                overflowY: 'auto',
                maxHeight: 'calc(80vh - 70px)', // Ajusta según tu diseño
              }}
            >
              {mensajes.map((msg) => (
                <ListGroup.Item
                  key={msg.id}
                  action
                  active={mensajeSeleccionado?.id === msg.id}
                  onClick={() => setMensajeSeleccionado(msg)}
                >
                  <div className="d-flex align-items-center">
                    
                    <div style={{ flex: 1 }}>
                      <strong>
                        {msg.titulo
                          ? msg.titulo.charAt(0).toUpperCase() +
                            msg.titulo.slice(1).toLowerCase()
                          : 'Título'}
                      </strong>
                        <div style={{ fontSize: '12px', color: '#555' }}>
                        {msg.descripcion  
                            ? msg.descripcion.length > 60
                            ? msg.descripcion.substring(0, 60) + '...'
                            : msg.descripcion
                            : ''}
                        </div>

                    </div>
                  </div>
                </ListGroup.Item>
              ))}
            </ListGroup>
          </Card>
        </Col>


        <Col md={8}>
          <Card style={{ height: '100%' }}>
            <Card.Header>
              {mensajeSeleccionado ? mensajeSeleccionado.nombre : 'Selecciona un mensaje'}
            </Card.Header>
            <Card.Body style={{ overflowY: 'auto' }}>
              {mensajeSeleccionado ? (
                <>
                  <p>
                    <strong>Título:</strong> {mensajeSeleccionado.titulo}
                  </p>

                  
                  <p>
                    <strong>Mensaje:</strong>
                  </p>
                  <p>{mensajeSeleccionado.descripcion}</p>
                </>
              ) : (
                <p>No has seleccionado ningún mensaje.</p>
              )}
            </Card.Body>
          </Card>
        </Col>
      </Row>
    </>
  );
}
